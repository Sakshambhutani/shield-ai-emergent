  ...
  return <div className="md-portfolio">
    ...
    <div className="md-portfolio-view" aria-label={view}>
      {view === 'Overview' && <>
        ...
        <section><Title title="Decisions required" note="Ordered by due date" /><div className="panel md-decision-ledger">{MD_DECISIONS.slice().sort((a,b) => a.due.localeCompare(b.due)).map(d => <details key={d.id} className="md-compact-decision"><summary><span className="md-ledger-date">{shortDate(d.due)}</span><span><strong>{d.title}</strong><span className="md-ledger-copy">{d.brief}</span><small>{d.owner}</small></span><span className="md-expand" aria-hidden="true">+</span></summary><div className="md-ledger-detail"><p>{d.request}</p><p>{d.consequence}</p><p><b>Close with:</b> {d.closure}</p><button onClick={() => navigate('Programmes',d.programme)}>Programme ↗</button></div></details>)}</div></section>
      ...
      {view === 'Engineering' && <>
        <section><Title title="Capacity & commitments" note={`${totals.assigned} assigned / ${totals.required} needed · entire portfolio`} /><div className="md-table-wrap panel"><table className="md-deals"><thead><tr>{['Workstream', 'Assigned / needed', 'Next deliverable', 'Dependency'].map(h => <th scope="col" key={h}>{h}</th>)}</tr></thead><tbody>{work.map(w => <tr key={w.id}><th scope="row">{w.name}</th><td><strong className={w.assigned < w.required ? 'amber' : ''}>{w.assigned} / {w.required}</strong></td><td>{w.deliverable}</td><td>{w.global}<details><summary>Delivery risk</summary><p>{w.risk}</p></details></td></tr>)}</tbody></table>{!work.length && <p className="md-empty">Team allocation follows the bid or paid-scope decision.</p>}</div></section>
