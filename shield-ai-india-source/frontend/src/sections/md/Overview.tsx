...
export default function Overview({ navigate }: { navigate: (area: Area, filter?: string)=>void }) {
 ...
 return <>
 <Cards>
  <Metric label="Delivery at risk" value={`${atRisk.length} milestones`} sub={`${signed(movement)} days WoW`} tone="mc-amber" onClick={()=>navigate('Delivery','At risk')}/>
  <Metric label="Critical hiring gaps" value={`${CRITICAL_JOINING_GAPS.length} roles`} onClick={()=>navigate('Hiring','Joining gaps')}/>
  <Metric label="Funding coverage" value={`${FUNDING_COVERAGE} months`} onClick={()=>navigate('Finance')} tone="mc-green"/>
  <Metric label="Budget variance · Sep" value={`${delta>0?'+':''}${(delta/budget*100).toFixed(1)}%`} sub={`₹${Math.abs(delta*100).toFixed(0)}L ${delta>=0?'over':'under'}`} tone={delta>0?'mc-amber':''} onClick={()=>navigate('Finance')}/>
 </Cards>
