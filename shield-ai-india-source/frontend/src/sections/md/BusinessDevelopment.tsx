...
export default function BusinessDevelopment({ initial = 'All' }: { initial?: string }) {
 ...
 <Metric label="Near closure · next 90 days" value={nearApplicable?inr(sumBd(near)):'—'} sub={nearApplicable?`${near.length} assumed late-stage deals · as of ${date(AS_OF)}`:`${year} is outside the current 90-day window`} onClick={()=>jump('bd-near-closure',{fy:year})}/>
 </Cards>
