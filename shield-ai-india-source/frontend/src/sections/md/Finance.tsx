...
import { Bars, Cards, Choice, Metric, Panel, Table } from './Shared';
...
export default function Finance() {
 ...
 return <>
 <nav className="mc-controls" aria-label="Finance financial year"><Choice label="Finance financial year" value={year} options={YEARS} onChange={selectYear}/></nav>
 <Cards>
<Metric onClick={()=>jump('finance-variance',{fy:year})} label="Budget variance" value={varianceText} sub={variance===null?'No spending yet':`Against ${inr(spending.periodBudget!)} period budget`} tone={metricTone(budgetTone(variance))}/>
<Metric onClick={()=>jump('finance-spending',{fy:year})} tone={metricTone(budgetTone(variance))} label="Spent to date / period budget" target={spending.periodBudget===null?undefined:inr(spending.periodBudget)} targetLabel="period budget" value={spending.spent===null?'N/A':inr(spending.spent)} sub={spending.spent===null?'Future year':`Estimated · ${spending.period}`}/>
...
<Metric onClick={()=>jump('finance-revenue',{fy:year})} label="Earnings plan" value={inr(plan.revenue+plan.tot)} sub={`${inr(plan.revenue)} revenue + ${inr(plan.tot)} ToT`}/>
 ...
 <Panel collapsible={false} id="finance-spending" title="Spending by FY"><div id="finance-variance" tabIndex={-1} style={{scrollMarginTop:16}} className="mc-strip">
  ...
  <span>Variance <b className={metricTone(budgetTone(variance))}>{varianceText}{variance!==null&&spending.periodBudget?` (${(Math.abs(variance)/spending.periodBudget*100).toFixed(1)}%)`:''}</b></span>
 ...
 <Panel id="finance-cash" title="Cash summary" context="Planning estimates: ₹40 Cr initial HQ funding; 50% of earnings collected in-year and 50% the next year. September uses half-year collections. Payments follow the spending estimates; future years are projections. These are not verified bank balances." aside={<span>{date(cash.asOf)}</span>}><Table headers={['Cash movement','Amount']}>
