...
const MISSION_COURSE = 'M80 160 H920 C1000 160 1000 402 920 402 H200 C90 402 90 644 200 644 H900';
const CHECKPOINTS = [
  { x: 80, y: 160 }, { x: 500, y: 160 }, { x: 920, y: 160 },
...
export default function Roadmap() {
  ...
  const [roadmapHeight, setRoadmapHeight] = useState(789);
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      const scale = fitToViewport
        ? Math.max(0.01, Math.min(1, (viewport.clientHeight - 8) / 851, viewport.clientWidth / 1600))
        : 1;
      setRoadmapScale(scale);
      setRoadmapHeight(fitToViewport
        ? Math.max(789, (viewport.clientHeight - 8) / scale - 62)
        : 789);
  ...
  return <Screen className={`company-roadmap${present ? ' company-roadmap-present' : ''}`}>
    ...
    <div ref={viewportRef} className="company-roadmap-scroll" role="region" aria-label="18-month mission corridor; scroll horizontally on smaller screens" tabIndex={0}>
      <div className="company-roadmap-canvas" data-testid="company-roadmap" style={{ zoom: roadmapScale, '--roadmap-height': `${roadmapHeight}px` } as CSSProperties}>
        <div className="mission-corridor">
          ...
          <div className="mission-path-area">
            <svg className="mission-path" viewBox="0 0 1000 669" preserveAspectRatio="none" aria-hidden="true">
              ...
              <path className="mission-center" d={MISSION_COURSE} />
            ...
            <ol className="company-milestones" aria-label="Company milestones">
              {MILESTONES.map((item, index) => {
                ...
                return <li key={item.month} className={`company-milestone above ${index === 0 ? 'first' : ''} ${index === MILESTONES.length - 1 ? 'last' : ''} ${selected === item.month ? 'selected' : ''}`} style={{ '--position': `${CHECKPOINTS[index].x / 10}%`, '--altitude': `${CHECKPOINTS[index].y / 669 * 100}%` } as CSSProperties}>
