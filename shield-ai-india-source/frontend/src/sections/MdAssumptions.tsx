...
import { DEFAULT_INPUTS, INPUT_REGISTER, MD_SOURCES, STRUCTURAL_ASSUMPTIONS, type PlanningInputs } from '@/data/md-planning';
...
export default function MdAssumptions({ open, inputs, onApply, onClose }: { open: boolean; inputs: PlanningInputs; onApply: (inputs: PlanningInputs) => void; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="md-detail md-assumptions" aria-labelledby="assumptions-title" onCancel={onClose} onKeyDown={e => e.stopPropagation()}>
    ...
    <div className="md-detail-body"><p className="md-register-intro">Public facts establish the programme and partnership scope. They do not establish prices, staffing, margins or realised performance. These inputs are a planning case, not an approved budget. Changes apply to this page for the current session.</p>
      ...
      <form onSubmit={e => { e.preventDefault(); const next = { ...inputs }; for (const item of INPUT_REGISTER) { const value = draft[item.key]; if (item.key === 'contract') next.contract = value === '' ? null : Number(value); else next[item.key] = Number(value); } onApply(next); onClose(); }}>
        <h3>Numerical inputs · all require validation</h3>
        {INPUT_REGISTER.map(item => <article className="md-assumption-input" key={item.key}><label htmlFor={`md-input-${item.key}`}>{item.label}<span>{item.unit}</span></label><input id={`md-input-${item.key}`} type="number" value={draft[item.key]} onChange={e => setDraft({ ...draft, [item.key]: e.target.value })} required={item.key !== 'contract'} min={item.min} max={item.max} step={item.key === 'headcount' || item.key === 'collectionLag' ? 1 : 'any'} placeholder={item.key === 'contract' ? 'Not disclosed — leave blank' : undefined} /><details><summary>Basis, sensitivity & validation</summary><p>{item.rationale}</p><p><b>Sensitivity:</b> {item.sensitivity}</p><p><b>Owner:</b> {item.owner}</p><p><b>Evidence needed:</b> {item.validate}</p></details></article>)}
      ...
      <h3>Model rules & exclusions</h3>{STRUCTURAL_ASSUMPTIONS.map(a => <details className="md-register-section" key={a.label}><summary>{a.label}</summary><p>{a.basis}</p><p><b>Owner:</b> {a.owner}</p><p><b>Validate:</b> {a.validation}</p></details>)}
