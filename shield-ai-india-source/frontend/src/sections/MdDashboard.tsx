...
export default function MdDashboard() {
  ...
  const risks = [
    ...(model.peakFunding > 0 ? [{ title: 'Funding', signal: `${dollars(model.peakFunding)} peak requirement`, action: 'Finance · secure funding before supplier commitments' }] : []),
  ...
  return <Screen className="md-dashboard md-numeric">
    ...
    <div className="md-numeric-toolbar"><span>Assumed scenario · USD millions</span><label className="md-scenario-select">Delivery timing<select aria-label="Delivery timing" value={delay} onChange={e => setDelay(Number(e.target.value))}><option value={0}>Planned acceptance</option><option value={6}>Acceptance delayed 6 months</option></select></label><button className="md-register-button" onClick={() => setAssumptionsOpen(true)} aria-haspopup="dialog">Assumptions ↗</button></div>
