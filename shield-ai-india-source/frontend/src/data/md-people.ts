...
// Illustrative month-two People and Culture review, independent of the October contract/finance snapshot.
...
export const PEOPLE_CRITICAL_ROLES = [
  ...
  { role: 'Customer programme manager', coverage: 'Existing operations team · covered', covered: true, due: '2026-12-01', action: 'Maintain delivery and acceptance ownership', owner: 'Programmes lead' },
...
export const PEOPLE_CAPABILITIES = [
  ...
  { capability: 'Customer trials and acceptance', primary: 'Existing programme manager', backup: 'No qualified second programme manager', covered: false, action: 'Cross-train a second operations team member', owner: 'Programmes lead', due: '2026-12-12' },
...
export const PEOPLE_EXIT_COUNT = HR_EXITS.filter(e=>e.date<=PEOPLE_AS_OF).length;
export const PEOPLE_HEADCOUNT = PEOPLE_BASE + PEOPLE_JOINED - PEOPLE_EXIT_COUNT;
...
export const PEOPLE_METRICS = [
  { name: 'Hiring vs plan', indicator: 'Lagging' as const, definition: 'Employees joined / planned joins due by the People and Culture review date. Six-month hiring progress and total headcount are shown separately.', value: `${PEOPLE_JOINED} / ${PEOPLE_JOINS_DUE}`, target: 'All planned joins due', context: `${PEOPLE_JOINS_DUE - PEOPLE_JOINED} joins overdue · ${PEOPLE_JOINED} / ${PEOPLE_TARGET - PEOPLE_BASE} six-month hires joined`, details: [{ label: 'Headcount', text: `${PEOPLE_HEADCOUNT} people; starting team ${PEOPLE_BASE}, target ${PEOPLE_TARGET}. ${PEOPLE_EXIT_COUNT} exit; replacement pending.` }, { label: 'Next action', text: 'Confirm joining dates for AUT-02 and STAFF-02; review interim cover with Engineering.' }] },
...
].map(metric => ({ ...metric, period: 'Illustrative People and Culture review · 30 Nov 2026' }));

export const PEOPLE_ASSUMPTION_ROWS: [string, string][] = [['Review', 'Constructed month-two People and Culture example at 30 Nov 2026; separate from the October contract and finance snapshot.'], ['Team plan', '15 existing employees + 25 planned hires = 40; six illustrative additional joins and one exit, example current team 20. One replacement restores the departed position and is separate from the 25 growth hires. Existing team split of 9 Hivemind, 3 BD and 3 operations is illustrative. Monthly planned joins: 4, 4, 5, 4, 4, 4.'], ['Evidence', 'All candidates, readiness sign-offs, coverage, actions and dates are illustrative; replace with People and manager records.'], ['Reporting', 'Published India job titles are used as examples throughout People and Culture. Illustrative position targets total 25 planned hires; public postings do not establish vacancy quantities or candidate progress. The chart counts candidates who reached each stage, including later stages. Each job has a different illustrative funnel, including candidates not selected or no longer pursuing the role. Cumulative stage totals retain that history even when hiring is complete. The table shows current interviewing and negotiation counts. Offers accepted includes joined staff plus accepted candidates awaiting joining; joined and awaiting joining are subsets, not additional acceptances. Six joined starters are included in the team, not additional to it.']];
